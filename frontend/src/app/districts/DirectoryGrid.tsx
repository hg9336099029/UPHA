  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
      {districts.map((row, index) => (
        <Link href={`/districts/${row.id}`} key={row.id} className="bg-white border border-gray-200 rounded-sm p-6 shadow-sm flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow">
          {row.logo ? (
