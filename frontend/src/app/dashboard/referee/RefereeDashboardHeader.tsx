...
export default function RefereeDashboardHeader() {
  const { authUser, meData, loading } = useAuth();
  const referee = meData as RefereeData | null;

  const firstName = authUser?.name?.split(" ")[0]?.toUpperCase() ?? "VIKRAM";
  ...
  return (
    <div className="mb-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 border-b border-gray-200 pb-4">
        <div>
          <div className="text-[10px] font-bold tracking-widest text-[#d97c55] uppercase mb-2">REFEREE DASHBOARD</div>
          <h1 className="font-heading text-5xl md:text-6xl font-bold uppercase tracking-wide text-primary">
            WELCOME BACK, <span className="text-[#d97c55]">{firstName}</span>
          </h1>
          <div className="bg-[#111827] inline-flex items-center text-white px-4 py-2 mt-4 text-[10px] font-bold tracking-widest uppercase rounded-sm shadow-sm">
            <span className="text-[#d97c55] mr-2">&bull;</span> REFEREE DASHBOARD
            <span className="mx-3 text-gray-600">|</span> {authUser?.name || "VIKRAM JOSHI"}
            <span className="mx-3 text-gray-600">|</span> RFR-{String(referee?.id ?? 45).padStart(5, "0")}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 shadow-sm rounded-sm p-6 flex flex-col justify-center border-l-4 border-l-emerald-500">
          <div className="text-[9px] font-bold tracking-widest text-gray-400 uppercase mb-2">ACCREDITATION</div>
          <div className={`font-heading text-2xl font-bold uppercase mb-1 ${referee?.paid ? "text-emerald-600" : "text-emerald-600"}`}>
            &bull; {referee?.paid ? "ACTIVE" : "PENDING"}
          </div>
          <div className="text-[11px] text-gray-500">Session 2025-26</div>
        </div>
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-sm p-6 flex flex-col justify-center border-l-4 border-l-[#d97c55]">
          <div className="text-[9px] font-bold tracking-widest text-gray-400 uppercase mb-2">REFEREE ID</div>
          <div className="font-heading text-xl font-bold uppercase text-primary mb-1">
            RFR-{String(referee?.id ?? 45).padStart(5, "0")}
          </div>
          <div className="text-[11px] text-gray-500">{referee?.district ?? "Varanasi"} District</div>
        </div>

        <div className="bg-white border border-gray-200 shadow-sm rounded-sm p-6 flex flex-col justify-center border-l-4 border-l-[#d97c55]">
          <div className="text-[9px] font-bold tracking-widest text-gray-400 uppercase mb-2">HIGHEST LEVEL OFFICIATED</div>
          <div className="font-heading text-2xl font-bold uppercase text-primary mb-1 truncate">{referee?.highest_level_officiated || "STATE"}</div>
          <div className="text-[11px] text-gray-500">Career highest</div>
        </div>
